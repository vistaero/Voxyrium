...
public class RenderDataFactory {
    ...
    private void acquireNeighborData(WorldSection section, int msk) {
        ...
        if ((msk&16)!=0) {//-z
            ...
            var raw = sec._unsafeGetRawDataArray();
            for (int i = 0; i < 32*32; i++) {
                this.neighboringFaces[i+32*32*4] = raw[Java17Compat.expand(i,0b11111_00000_11111)|(0x1F<<5)];//pull the +z faces from the section
            }
            sec.release(WorldSection.RELEASE_HINT_POSSIBLE_REUSE);
        }
        if ((msk&32)!=0) {//+z
            ...
            var raw = sec._unsafeGetRawDataArray();
            for (int i = 0; i < 32*32; i++) {
                this.neighboringFaces[i+32*32*5] = raw[Java17Compat.expand(i,0b11111_00000_11111)];//pull the -z faces from the section
            }
            sec.release(WorldSection.RELEASE_HINT_POSSIBLE_REUSE);
        }
    }
        ...
        for (int y = 0; y < 32; y++) {
            ...
            for (int z = 0; z < 32; z++) {
                ...
                if (z == 30 && partialHasCount != 0) {//Hackfix for incremental count overflow issue
                    ...
                    //Clear the sum
                    sumA &= ~(Java17Compat.expand(Integer.toUnsignedLong(partialHasCount), X_I_MSK)*0x1F);
                    sumB &= ~(Java17Compat.expand(Integer.toUnsignedLong(partialHasCount)>>11, X_I_MSK)*0x1F);
                    sumC &= ~(Java17Compat.expand(Integer.toUnsignedLong(partialHasCount)>>22, X_I_MSK)*0x1F);
                }
                ...
                {//Dont need this as can just increment everything then -1 in mask
                    ...
                    sumA += Java17Compat.expand(imsk, X_I_MSK);
                    sumB += Java17Compat.expand(imsk>>11, X_I_MSK);
                    sumC += Java17Compat.expand(imsk>>22, X_I_MSK);
                }*/
